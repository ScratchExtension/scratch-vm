...
class Scratch3NewBlocks {
    ...
    getInfo () {
        return {
            ...
            blocks: [
                ...
                {
                    opcode: 'jsonDict',
                    text: 'parse [JSONDATA] as JSON and load [KEY]',
                    blockType: BlockType.REPORTER,
                    arguments: {
                        JSONDATA: {
                            type: ArgumentType.STRING,
    ...
    /**
     * Get the browser.
     * @return {number} - the browser name.
     */
    getBrowser () {
    ...
    ajaxRequest (args){
        ...
        ajaxPromise.then(result => result);
        return ajaxPromise;
    }
    ...
    power (args) {
        return Math.pow(Cast.toNumber(args.ONE), Cast.toNumber(args.TWO));
    }

    windowOpen (args) {
        const regex = /(^https?:\/\/(?:[\w-]+\.)*(?:scratch\.mit\.edu|scratch-wiki\.info)(?:\/(?:\S*[\w:/#[\]@$&'()*+=])?)?(?![^?!,:;\w\s]\S))/;
        if (regex.test(Cast.toString(args.URL))) {
            window.open(Cast.toString(args.URL));
        }
    }

    jsonDict (args) {
        const j = JSON.parse(Cast.toString(args.JSONDATA));
